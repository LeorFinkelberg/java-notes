...
Возможность передавать как явный массив, так и плоский список аргументов полезна для эволюции кода: вы можете улучшить обычный метод с параметром-массивом, превратив его в метод с переменным количеством аргументов, и это изменение не нарушит существующие точки вызовов. Тем не менее такое удобство имеет свою цену. Когда вы передаете только один из аргументов, вы не можете сразу понять, обернут он в массив или нет. Пример
```java
printAll(obj);
```
Здесь непонятно, является ли `obj` массивом. Все зависит от типа: если это массив объектов, то он не будет обернут. ==Очень важно понимать, что решение принимается на основании объявленного, а не фактического типа==
```java
...
printAll(obj);
```
Переменная `obj` содержит массив объектов, так что можно было бы ожидать, что оборачиваться она не будет. ==Тем не менее тип переменной объявлен просто как `Object`, не как массив==. Решение о том, будет ли объект обернут в массив, выполняется во время компиляции, а компилятору неизвестен реальный тип объекта, хранящегося внутри переменной `obj`. Поэтому он решает обернуть массив в еще один массив [[Список литературы#^f416f7]]<c. 89>

NB! Согласно спецификации Java (JLS 15.12.4.2), аргумент не оборачивается в массив, если он может быть присвоен к переменной соответствующего типа массива. Поскольку `null` может быть присвоен к любой переменной массива, он не оборачивается в массив [[Список литературы#^f416f7]]<c. 89>

Для того чтобы преобразовать `float[]` в `Float[]` нельзя использовать `Arrays.stream()`, так как он умеет работать только с `int`, `long` и `double`.

Варианты решения
```java
// Если уже используется Apache Commons
import org.apache.commons.lang3.ArrayUtils;

public static float getKthStatistic(float[] array, int k) {
  Float[] boxedArray = ArrayUtils.toObject(array); // <- NB!
  ...
}

// Если нет зависимости
float[] arr = {2.0f, 3.5f, 4.1f};

	Float[] boxedArr = IntStream.range(0, arr.length)
    .mapToObj(idx -> arr[idx])
	.toArray(Float[]::new);
```

NB! ==Пользуйтесь ссылками на методы с осторожностью==, особенно когда они указывают на метод или конструктор, который перегружается или имеет неопределенное количество параметров. Вы можете случайно сослаться на неверный метод или передать больше аргументов, чем предполагалось [[Список литературы#^f416f7]]<c. 110>

Забавная алгоритмическая задача. Пусть нужен метод для анализа форматированной строки. Метод должен возвращать `true`, если форматированная строка содержит спецификатор формата `%d`. Пользоваться `str.contains("%d")` нельзя, так как возможно также экранировать символ `%`, удвоив его, поэтому для ввода `%%d` нужно возвращать `false`, а для `%%%d` -- `true`
```java
boolean hasPercentD(String text) {
	int pos = 0;
	while (true) {
	    pos = text.indexOf('%', pos); // здесь как будто срез text[pos:]
...
```python
def has_percent_d(text: str) -> bool:
    pos = 0
    while True: 
		try:
	        pos: int = text.index('%')
...
Еще одна характерная для Java проблема инициализации возникает, когда переопределенный метод вызывается из конструктора суперкласса. К примеру рассмотрим класс, который представляет сущеность с числовым идентификатором [[Список литературы#^f416f7]]<c. 147>
```java
class Entity {
    final int id;
...
```
Здесь инициализация поля `random` фактически является частью конструктора класса `RandomIdEntity`, который выполняется после конструктора суперкласса. В сущности, этот класс эквивалентен следующему
```java
class RandomIdEntity extends Entity {
    final Random random;

    RandomIdEntity() {
        super();  // конструктор суперкласса вызывается ДО инициализации random
        random = new Random();
    }
...
```
==Когда выполняется конструктор суперкласса, поля подкласса еще не инициализированны==. Тем не менее мы вызываем переопределенный метод `generateId()`, который предполагает, что полю `random` уже присвоено какое-то начальное значение. 

NB! В целом старайтесь сделать конструктор настолько простым, насколько это вообще воозможно. Если вам нужно воспользоваться какими-то вспомогательными методами, объявите их как _статические_, _приватные_ или _финальные_, таким образом подклассы не смогут их переопределить.

Проблема иного рода возникает, когда инициализация двух классов зависит друг от друга
```java
class A {
...
Инициализаторы полей обычно выполняются в том порядке, в каком поля заявлены в классе, так что, если мы поменяем местами поля `unused` и `WELCOME`, запуск класса `A` даст на выходе `HELLO`. Тем не менее существуют одно исключение. Если инициализатором поля является _константное выражение примитивного типа_ или _типа `String`, вычисляемое только во время компиляции программы_, тогда ==поле будет инициализироваться во время загрузки класса, до того как начнется инициализация==. Если мы уберем из примера вызов `toUpperCase()`, то увидим, что `null` больше не выводится, поскольку поле `WELCOME` инициализировано во время загрузки класса [[Список литературы#^f416f7]]<c. 151>.

NB! Полезные утилиты для мониторинга ресурсов: `btop`, `glances`
```bash
brew install btop
```
А с помощью утилиты `jstack` можно поизучать зависший процесс Java

NB! Если поле `static`, делайте его `final`!

==NB! Для объектов `final` означает, что ССЫЛКУ нельзя изменить, а не сам объект!==
```java
class MyClass {
  // final означает, что counter НЕ СМОЖЕТ ссылаться на другой объект
  private static final AtomicInteger counter = new AtomicInteger(0);
  
  public static void main(String[] args) {
    // counter = new AtomicInteger(100); ОШИБКА!!!
    counter.addAndGet(10); // counter += 10;
	counter.incrementAndGet(1);
  }
}
```
То есть получается, что для изменяемых объектов `final` запрещает изменять ссылку на объект, но сам объект по-прежнему изменяемый. В случае неизменяемых объектов `final` также запрещает изменять ссылку, а сам объект и так был неизменяемый.

NB! То есть получается, что ==static-поля в классе (что-то вроде атрибутов класса в Python) разделяются между всеми экземплярами класса==. В отличие от атрибутов экземпляра класса, которые имеют свои значения для каждого экземпляра. Пример
```java
class Employee {
    private static int employeeCounts = 0; // по сути атрибут класса; разделяется между экземплярами
    // атрибут экземпляра класса; свое значение у каждого экземпляра
    private String name;  
    private String email;
    private int age;

    public Employee(String name, String email, int age) {
        this.name = name;  // self._name = name
        this.email = email;
        this.age = age;
    }
...
var leorFinkelberg = new Employee("Leor Finkelberg", "leor.finkelberg@yandex.ru", 39);
var alexanderPodvoyskiy = new Employee("Alexander Podvoyskiy", "apodvoyskiy08@gmail.com", 25);

leorFinkelberg.getEmployeeCounts();  // 2
alexanderPodvoyskiy.getEmployeeCounts();  // 2
```
Главное правило: `static` принадлежит КЛАССУ (одно на всех, живет пока класс загружен). Non-static принадлежит экземпляру класса (объекту) (свое у каждого, живет пока жив объект).

Оказывается, что в Java тип целевой переменной не используется при определении типа математического выражения, так что даже если мы присвоим результат переменной типа `long`, умножение будет выполняться со значением типа `int`. Пример
```java
int seconds = 50_000;
long res = seconds * 1_000_000;  // -1539607552; ПЕРЕПОЛНЕНИЕ
long res = (long) (seconds * 1_000_000);  // -1539607552; ПЕРЕПОЛНЕНИЕ; даже явное приведение не работает

long res = seconds * 1_000_000L;  // OK
```
NB! Если вы собираетесь перемножить два числа и присваиваете результату тип `long`, проверьте, чтобы хотя бы один из операндов также имел тип `long`, чтобы избежать возможного числового переполнения.

Для работы со временем можно использовать перечисление
```java
long microseconds = TimeUnit.SECONDS.toMicros(seconds);
```

Иногда возникают проблемы совместимости. В таком случае хороший выход -- объявить перегруженный метод и пометить исходный как устаревший
```java
@Depricated
...
```
Теперь более старые клиенты могут работать как раньше, но у вас появятся предупреждения об устаревшем методе в местах вызова, где используются аргументы, не являющиейся строкой. Начиная с Java 9, вы можете добавить промежуточное более сильное предупреждение об устаревшем методе, использовав `@Deprictated(forRemoval=true)`, чтобы подчеркнуть намерение удалить этот метод в более поздних версиях.
...
```
В Java _обобщенные типы_ стираются ==во время выполнения программы==. Из-за исторических причин параметризованные типы существуют и проверяются только в исходном коде. Когда программа на Java выполняется , аргументы типа любого параметризованного значения перестают быть известными, и любое вхождение параметра типа наподобие `E` заменяется его верхней границей (или `Object`, если верхняя граница явно не объявлена). Так что с точки зрения виртуальной машины `List<E>` или `List<String>` не существуют, есть только `List`. Виртуальная машина Java понятия не имеет о типе `E`, и метод `.get()` просто возвращает `Object`. Чтобы преобразовать `Object` в `String`, компилятор Java вставляет скрытое преобразование типа, преобразуя метод в следующий вид [[Список литературы#^f416f7]]<c. 236>
```java
...
NB! Не стоит использовать вычитание в `compareTo()`
NB! Сравнивая числа с плавающей точкой, не забывайте о возможнных значениях NaN. Метод `Double.compare()` обрабатывает особые числа, такие как -0.0 и NaN [[Список литературы#^f416f7]]<c.329> Как и с булевыми полями, гораздо лучше воспользоваться библиотечными методами: `Integer.compare()`, `Boolean.compare()`.

Сравнение объектов [[Список литературы#^f416f7]]<c. 331>
```java
...
public int compareTo(Connection that) {
  return ComparisonChain.start()
    .compareTrueFirst(this instanceof SecureConnection),
	                 (that instanceof SecureConnection)
...
record User(String name) implements Comparable<User> {
  static final Comparator<User> USER_ORDER = 
      comparing(User::name, nullLast(naturalOrder()));
...
NB! Любой ценой избегайте коллекций типов `Byte` и `Short` [[Список литературы#^f416f7]]<c. 342>
